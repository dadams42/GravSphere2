...
vdata = '1krltr_vels_plumcuspom.txt' #1k tracers (LOS + PMs)
#vdata = '100rltr_vels_plumcuspom.txt' #100 tracers (LOS + PMs)
#vdata = '10krltr_vels_plumcuspom.txt' #10k tracers (LOS + PMs)
...
R, surfden, surfdenerr = np.loadtxt(str(dirf + 'photplumcusp.txt')) #photometric tracer profile (binned)
...
Rhalf = 0.11602320464092819 #half-light radius (doesn't need to be exact, but rather used as a reference scale)
...
pfits = np.array([203.7268193701383, 0.1, 2, 5, 0.1]) #reference values of alpha-beta-gamma profile obtained from pre-fitting (rho0, r0, alp, bet, gam) this can be #done e.g. with binulator, directly fitting or manually (e.g. looking at a Plummer profile matching central density and ~ Rhalf for the scale radius)
